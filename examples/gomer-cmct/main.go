...
const (
	...
	defaultTimeout = 40 * time.Minute // This program has in +/- 10ms accuracy due to its structure.
...
func main() {
	...
	{ // Everything has been taken care of, this is the core code:
		...
		writeln(writer, fmt.Sprintf("CMCT took %v value: %v (%d nodes)", elapsed, result.Score(), mcs.NodeCount()))
		flush(writer)
